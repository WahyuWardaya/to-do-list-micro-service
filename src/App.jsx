      ...
      <div className="bg-gray-800 rounded-xl shadow-2xl p-8 w-full max-w-lg border border-gray-700">
        ...
        <ul className="space-y-4">
          {todos.map((todo, index) => (
            <li key={index} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg border border-gray-600 transition-transform transform hover:scale-[1.01]">
              {isEditing === index ? (
                // Mode Edit
                <div className="flex-1 flex items-center gap-2">
                  ...
                  />
                  <button onClick={() => saveEdit(index)} className="text-green-400 hover:text-green-600">
                ...
                <>
                  <span className={`flex-1 cursor-pointer select-none text-lg ${todo.completed ? 'line-through text-gray-500' : 'text-gray-200'}`} onClick={() => toggleTodo(index)}>
                    {todo.text}
                  </span>

                  <div className="flex items-center gap-2">
                    <button onClick={() => startEdit(index, todo.text)} className="text-yellow-400 hover:text-yellow-600 transition-colors duration-200">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                      </svg>
                    </button>
                    <button onClick={() => deleteTodo(index)} className="text-red-400 hover:text-red-600 transition-colors duration-200">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
